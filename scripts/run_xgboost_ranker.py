...
def print_fold_distribution(df, n_folds):
    ...
    for fold_i in range(n_folds):
        ...
        # For continuous relevance, show distribution statistics
        print(f"  📊 Relevance stats:")
        print(f"    Min: {fold_data['relevance'].min():.4f}")
        print(f"    Max: {fold_data['relevance'].max():.4f}")
        print(f"    Mean: {fold_data['relevance'].mean():.4f} ± {fold_data['relevance'].std():.4f}")
        
        # Count how many measured vs unlabeled if label_PNU exists
        if 'label_PNU' in fold_data.columns:
            measured = fold_data[fold_data['label_PNU'] != -1].shape[0]
            unlabeled = fold_data[fold_data['label_PNU'] == -1].shape[0]
            print(f"  📊 Measured: {measured}, Unlabeled: {unlabeled}")
...
def calculate_mrr_continuous(y_true_grouped, y_score_grouped, threshold_percentile=75):
    """
    Calculate Mean Reciprocal Rank for continuous relevance values
    Uses percentile threshold to define "relevant" items
    """
    rr_scores = []
    for y_true, y_score in zip(y_true_grouped, y_score_grouped):
        ...
        # Define threshold for "relevant" (e.g., top 25% or above median)
        if len(y_true) > 3:
            threshold = np.percentile(y_true, threshold_percentile)
        else:
            threshold = y_true.max() * 0.8  # For small groups, use 80% of max
        
        # Check if any item meets the threshold
        if np.sum(y_true >= threshold) == 0:
            rr_scores.append(0.0)
        else:
            # Get rank of first item above threshold
...
def calculate_spearman_per_fold(y_true_grouped, y_score_grouped):
    """
    Calculate average Spearman correlation across groups
    """
    spearman_scores = []
    for y_true, y_score in zip(y_true_grouped, y_score_grouped):
...
def print_final_ranking_summary(fold_results, output_dir):
    ...
    # Include spearman in metrics if available
    metrics = ['ndcg_1', 'ndcg_3', 'ndcg_5', 'ndcg_10', 'mrr']
    if 'spearman' in fold_results[0]:
...
def main():
    ...
    parser.add_argument('--use_continuous_relevance', action='store_true', default=True,
                       help='Use continuous enzyme activity values for ranking')
    ...
    print("="*70)
    print(f"📊 Using {'continuous' if args.use_continuous_relevance else 'binary'} relevance scores")
    ...
    df, feature_cols, cfg = load_and_prepare_data(
        ...
        use_continuous_relevance=args.use_continuous_relevance
    )
    ...
    # Check what relevance column we have
    if 'relevance' in df.columns:
        relevance_type = "continuous" if df['relevance'].nunique() > 10 else "binary"
        print(f"\n📋 Using {relevance_type} relevance scores for ranking:")
        print(f"  Range: [{df['relevance'].min():.4f}, {df['relevance'].max():.4f}]")
        print(f"  Mean: {df['relevance'].mean():.4f} ± {df['relevance'].std():.4f}")
        print(f"  Unique values: {df['relevance'].nunique()}")
    else:
        print("\n⚠️  No relevance column found! Check your data loading.")
        sys.exit(1)
    ...
    for fold_i in range(n_folds):
        ...
        # Add MRR (with continuous adaptation)
        if args.use_continuous_relevance:
            metrics['mrr'] = calculate_mrr_continuous(y_true_groups, y_score_groups)
            metrics['spearman'] = calculate_spearman_per_fold(y_true_groups, y_score_groups)
        else:
            metrics['mrr'] = calculate_mrr_continuous(y_true_groups, y_score_groups, threshold_percentile=100)  # Only count perfect scores as relevant
    ...
    if all_protein_performances:
        ...
        for metric in ['ndcg_1', 'ndcg_3', 'ndcg_5', 'ndcg_10', 'spearman', 'pearson', 'reciprocal_rank']:
        ...
        if 'protein' in all_protein_df.columns:
            # Use different aggregation for continuous vs binary
            agg_dict = {
                'ndcg_5': ['mean', 'std'],
                'spearman': 'mean',
                'fold': 'count'
            ...
            # Add n_positives only if it exists (binary case)
            if 'n_positives' in all_protein_df.columns:
                agg_dict['n_positives'] = 'first'
            # Add true_mean for continuous case
            if 'true_mean' in all_protein_df.columns:
                agg_dict['true_mean'] = 'first'
            
            protein_consistency = all_protein_df.groupby('protein').agg(agg_dict).round(4)
            
            # Flatten column names
            protein_consistency.columns = ['_'.join(col).strip() for col in protein_consistency.columns.values]
            protein_consistency = protein_consistency.sort_values('ndcg_5_mean', ascending=False)
            ...
            for prot, row in protein_consistency.head(10).iterrows():
                # Print the column names to debug (you can remove this after confirming)
                # print(f"DEBUG - Available columns: {row.index.tolist()}")
                
                # Check if this is continuous case by looking for true_mean in column names
                if any('true_mean' in col for col in row.index):
                    # Find the actual column name containing 'true_mean'
                    true_mean_col = [col for col in row.index if 'true_mean' in col][0]
                    print(f"  {prot}: NDCG@5={row['ndcg_5_mean']:.3f} (±{row['ndcg_5_std']:.3f}) "
                        f"| Spearman={row['spearman_mean']:.3f} | Mean activity={row[true_mean_col]:.3f}")
                else:
                    # Binary case - look for n_positives column
                    if any('n_positives' in col for col in row.index):
                        n_pos_col = [col for col in row.index if 'n_positives' in col][0]
                        print(f"  {prot}: NDCG@5={row['ndcg_5_mean']:.3f} (±{row['ndcg_5_std']:.3f}) "
                            f"| Spearman={row['spearman_mean']:.3f} | Positives={int(row[n_pos_col])}")
                    else:
    ...
    model_info = {
        ...
        'relevance_type': 'continuous' if args.use_continuous_relevance else 'binary'
    }
